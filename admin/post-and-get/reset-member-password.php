...
if (empty($nic_number)) {
    header('Location: ../forget-password-member.php?message=19');
    exit();
}
...
if ($MEMBER->CheckNicNumber($nic_number)) {
    ...
    if ($MEMBER->GenarateCodeMember($nic_number)) {
        ...
        if (mail($email, $subject, $html, $headers)) {
            header('Location: ../reset-member-password.php?message=12');
        } else {
            header('Location: ../reset-member-password.php?message=14');
        }
    }
    exit();
} else {
