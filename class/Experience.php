...
class Experience {
    ...
    public function GetExperienceByMember($member) {

        $query = "SELECT experience.id, experience.skill_detail, experience.description, experience.working_place, experience.duration, experience.sort  FROM `experience` join `skill_details` ON experience.skill_detail = skill_details.id join `member` ON skill_details.member = member.id WHERE member.id = '" . $member . "' ORDER BY  experience.sort  ASC";
